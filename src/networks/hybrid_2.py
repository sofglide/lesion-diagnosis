...
class Hybrid2(BaseNet):
    """
    Multiple pretrained models and a different augmented image for each model
    """
    ...
    @staticmethod
    def _get_model(num_classes: int, params: Optional[Dict[str, Any]] = None) -> Tuple[nn.Module, nn.Module]:
        model = _Hybrid(num_classes, params)
        return model, model.fusion_layers
...
class _Hybrid(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        :return:
        """
        x = torch.cat([embedder(x[:, i, :]) for i, embedder in enumerate(self.embedders.values())], dim=1)
